...
import {
	APPLICATIONSTATUS, APPLICATIONTYPES, SELECTSTYLE,
	AppDataStyle,
...
export default function PDHSAdmin() {
  ...
  useEffect(() => {		
	...
	async  function getAllApplication() {
		try {
			let myUrl = `${process.env.REACT_APP_AXIOS_BASEPATH}/application/list`;
			let resp = await axios.get(myUrl);
			//console.log(resp.data);
			setApplicationMasterArray(resp.data);
			setApplicationArray(resp.data.filter(x => x.owner === DEFAULTOWNER));
		} catch (e) {
			console.log(e);
		}	
	}
	...
	function filterArray(myArray, item) {
		...
		switch (item) {
			...
			case "PJYM":
			case "HUMAD":
				tmpArray = myArray.filter(x => x.owner === item);
				break;
			default:
				tmpArray = myArray;
				break;
	...
	<Grid className={gClasses.noPadding} key="AllPatients" container align="center">
		...
		<DisplayFunctionItem item="Admin" />
	</Grid>	
	</Box>
	)}
	...
	{(isDrawerOpened !== "") &&
		...
		{(editApplRec.desc === APPLICATIONTYPES.splitFamily) &&
			...
			<br />
			{memberNames.map(m => 
				<Typography className={gClasses.patientInfo2Blue}>{m}</Typography>
			)}
			<TextValidator required fullWidth className={gClasses.vgSpacing}
				label="Admin Remarks" type="text"
				value={emurRemarks}
