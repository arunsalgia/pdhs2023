...
export default function Application(props) {
  ...
  useEffect(() => {		
	...
	async  function getAllApplication() {
		try {
			...
			if (userType === 'user') {
				...
				if ((adminInfo & ADMIN.superAdmin) === 0) {
					if ((adminInfo & ADMIN.prwsAdmin) === 0)
						tmpArray = tmpArray.filter(x => x.owner !== "PRWS");
					if ((adminInfo & ADMIN.pjymAdmin) === 0)
						tmpArray = tmpArray.filter(x => x.owner !== "PJYM");
					if ((adminInfo & ADMIN.humadAdmin) === 0)
						tmpArray = tmpArray.filter(x => x.owner !== "HUMAD");
				}
				setApplicationMasterArray(tmpArray);
				let xxx = filterArray(tmpArray, DEFAULTOWNER);
				setApplicationArray(xxx);
			}
	...
	<div>
		...
		{(!isMobile()) &&
		<Grid align="center" item xs={7} sm={7} md={3} lg={3} >
			<Typography className={gClasses.patientInfo2Brown} >{(userType === 'user') ? 'Admin' : 'Applicant'}</Typography>
		</Grid>
		}
		{(!isMobile()) &&
		...
		{applicationArray.map( (a, index) => {
			...
			{(!isMobile()) &&
			<Grid align="center" item xs={7} sm={7} md={3} lg={3} >
				<Typography className={gClasses.patientInfo2}>{(userType === 'user') ? a.userName : a.name}</Typography>
			</Grid>
			}
			{(!isMobile()) &&
			<Grid align="center" item xs={2} sm={2} md={1} lg={1} >
	...
	<CssBaseline />
	<DisplayPageHeader headerName={"Application Status " + ((userType === 'user') ? '(User)' : '(Admin)')  } groupName="" tournament=""/>
	{(userType !== 'user') &&
