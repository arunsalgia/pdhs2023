...
export default function MemberPersonal(props) {
  ...
  function handleResize() {
    ...
    if ((dispType === "xs") || (dispType === "sm")) {
      myInfo +=  "Age:     : " +  dispAge(m.dob, m.gender) + "<br />";
      myInfo +=  "Relation : " +  myRelation + "<br />";
      myInfo +=  "Mar. Sts : " +  m.emsStatus + "<br />";
      myInfo +=  "Blood Grp: " +  m.bloodGroup + "<br />";
    } 
    //console.log("Call Email");
    var tmp = dispEmail(m.email);
    //console.log("EM: "+tmp);
    if (tmp != "")  myInfo +=  "Email Id : " +  tmp;
    //if (m.email1 !== "") myInfo +=  "<br />" + "Email Id1: " +  dispEmail(m.email1);
  
		return (
		<Box  key={"MEMBOX"+index} style={{marginLeft: "5px", marginLeft: "5px" }} className={((index % 2) == 0) ? gClasses.boxStyleEven : gClasses.boxStyleOdd} borderColor="black" borderRadius={30} border={1} >
		<Grid key={"MEMGRID"+index} className={gClasses.noPadding} key={"SYM"+index} container justify="center" alignItems="center" >
			<Grid align="left" item xs={7} sm={7} md={4} lg={3} >
				<Typography>
          <span style={{marginLeft: "0px", paddingLeft: "0px" }} className={(index === 0) ? gClasses.patientInfo2Brown : gClasses.patientInfo2Blue }>{getMemberName(m)+((!isMobile()) ? " ("+dispAge(m.dob, m.gender)+")" : "") }</span>
      ...
      {((dispType !== "xs") && (dispType !== "sm"))  &&
			<Grid align="center" item md={1} lg={1} >
				<Typography className={gClasses.patientInfo2}>{myRelation}</Typography>
			</Grid>
			}
      {((dispType !== "xs") && (dispType !== "sm"))  &&
			<Grid align="left" item md={1} lg={1} >
      ...
      {((dispType !== "xs") && (dispType !== "sm"))  &&
        ...
        <Grid align="left" item md={2} lg={2} >
          <Typography className={gClasses.patientInfo2}>
            <span>{m.patientInfo2}{dispEmail(m.email)}</span>
            <span className={gClasses.patientInfo2}>{m.patientInfo2}{dispEmail(m.email1)}</span>
           </Typography>
        </Grid>
			}
      <Grid align="center"  item xs={1} sm={1} md={1} lg={1} >
          {(edit) &&
            <span className={gClasses.patientInfo2}><VsRadio checked={radioRecord === index} onClick={() => setRadioRecord(index)}  /></span>
          }
      </Grid>
		</Grid>
		</Box>
	...
	return(
		<div>
		{memberArray.map( t =>
			<Tooltip align="left"  className={gClasses.patientInfo2Brown} key={"MEMBER"+t.mid} type="info" effect="float" id={"MEMBER"+t.mid} multiline={true}/>
		)}
		</div>
	)}
	...
	{((isDrawerOpened === "ADDPERSONAL") || (isDrawerOpened === "EDITPERSONAL")) &&
		...
		<br />
		<br />
		<Grid key="Name" className={gClasses.noPadding} container justify="center" alignItems="center" >
			<Grid  align="left" style={{marginTop: "12px" }}  item xs={3} sm={3} md={1} lg={1} >
        ...
        <TextValidator required className={gClasses.vgSpacing} inputProps={{className: gClasses.dateTimeNormal}}
